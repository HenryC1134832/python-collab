...
class MS_SL2_model(nn.Module):
    def __init__(self,
        ...
        #weight for each branch
        weight = [0.5+0.001, 0.5-0.001]
        
        #self.wList = [0.5+0.001, 0.5-0.001]
        self.wList = th.Tensor(weight)
        print("weight initialization",self.wList)
        self.PRelu = nn.PReLU()
        # output 1x1 conv
        # n x B x T => n x N x T
        # NOTE: using ModuleList not python list
    ...
    def get_slice(self):
        cpt = th.load('/media/lab70809/Data02/MS_R1_SL1_saved_models/48.pt.tar')
...
class MS_SL2_split_model(nn.Module):
    ...
    def forward(self, x):
        ...
        for Slice in range(self.slice):
            ...
            total_connection = skip_connection * self.wList[Slice] + total_connection
            #print('total connection ', total_connection)
            if __name__ == "__main__":
                #print('slice weight', self.wList[Slice])
                #print('total_connection', total_connection.size())
        ...
        return [self.decoder_1d(x, squeeze=True) for x in s]
#%%        
#***************************Model for Changing Weight**************************
class MS_SL2_Weight(nn.Module):
    def __init__(self,
                 L=16,
                 N=512,
                 X=8,
                 ...
                 non_linear="sigmoid",
                 causal=False):
        super(MS_SL2_Weight, self).__init__()
        supported_nonlinear = {
            "relu": F.relu,
            "sigmoid": th.sigmoid,
            "softmax": F.softmax
        ...
        #self.wList = self._build_weight(Slice)
        #self.wList = nn.Parameter(th.tensor([0.5+0.001, 0.5-0.001]),requires_grad=True)
        # self.wList= th.nn.Parameter(th.randn(2, requires_grad=True, dtype=th.float))
        self.wList= th.nn.Parameter(th.tensor([0.5+0.001, 0.5-0.001, 0.4+0.001, 0.4-0.001], requires_grad=True, dtype=th.float))
        #self.wList = nn.Parameter(th.tensor([1,1]), requires_grad=True) 
        self.PRelu = nn.PReLU()
        # output 1x1 conv
        # n x B x T => n x N x T
        # NOTE: using ModuleList not python list
    ...
    #def _build_weight(self, slices):
    #    wList = nn.Parameter(th.randn(slices),requires_grad=True)
    #    return wList    
    def _build_blocks(self, num_blocks, **block_kwargs):
        """
        Build Conv1D block
            ...
            for i in range(self.R):
                for j in range(self.X):
                    ...
                    if __name__ == "__main__":
                        print('finished 1D Conv block skip_connection size', skip.size())
                        print('finished 1D Conv blick ouput size', y.size())
            print('skip connection',skip_connection)            
            wlist_T = (self.wList.reshape(1,-1).t())
            print(wlist_T)
            print("tcn * weight",skip_connection * wlist_T[Slice] )
            tcn_weight = skip_connection * wlist_T[Slice]
            #print('output of TCN mupltiply with weight', tcn_weight)
            #print("TCN*Weight", tcn_weight)
            total_connection = tcn_weight + total_connection
            #print('total tcn output using in slice', total_connection)
                #print('value of total connection', total_connection)
            if a==0:
                result= total_connection
                print('first loop ', total_connection)
                a=a+1
            else:
                print('second loop', total_connection)
                result= result + total_connection
            if __name__ == "__main__":
                # print('slice weight', self.wList[Slice][0][1])
                print('slice weight', self.wList[Slice].item())
                print('total_connection', total_connection.size())
...
def foo_conv1d_block():
    ...
    print(param(nnet))
#%%
#**********Fuction for calling the layernorm **********************************
def foo_layernorm():
    C, T = 256, 20
...
#%%
#**********Fuction for calling the ConvTasNet Model****************************
def foo_conv_tas_net():
    ...
    print(s1.shape)
#%%    
#**********Fuction for calling the encoder_decoder*****************************    
def foo_encoder_decoder():
    N,L = 512,16
    x = th.rand(4, 1000)
    nnet = Conv1D(1, N, L, stride=L // 2, padding=0)
    # print(nnet)
    print("encoder #param: {:.2f}".format(param(nnet,Mb=False)))
    x = nnet(x)
    s1 = x[0]
    print(s1.shape)
    
    nnet = ConvTrans1D(N, 1, kernel_size=L, stride=L // 2, bias=True)
    # print(nnet)
    print("decoder #param: {:.2f}".format(param(nnet,Mb=False)))
    x = nnet(x)
    s1 = x[0]
    print(s1.shape)

 #%%   
#**********Fuction for calling the MB_ConvTasNet Model************************* 
def foo_MB_ConvTasNet():
    ...
    print(s1.shape)
 #%%   
#**********Fuction for calling the weight Model********************************
def foo_MS_SL2():
    x = th.rand(4, 1000)
...
#%%
#**********Fuction for calling the MS_SL2_split_model Model********************      
def SL2_split():
    x = th.rand(4, 1000)
    ...
    print(s1.shape)
    #cpt = th.load('/media/lab70809/Data02/MS_R1_SL2_TL_AIcenter_saved_models/15.pt.tar')
...
#**********Fuction for calling the MB_ConvTasNet Model*************************
def foo_TCN():
    cpt = th.load('/media/lab70809/Mac-pro/Speech_Denoiser/MS_R1_SL1_saved_models/best.pt.tar')
    nnet=MB_ConvTasNet()
    nnet.load_state_dict(cpt["model_state_dict"])
    print("Here is slices info",nnet.slices)
    print("TCN #param: {:.2f}".format(param(nnet.slices,Mb=False)))
...
def SL_weight():
    x = th.rand(4, 1000)
    nnet = MS_SL2_Weight(norm="cLN", causal=False)
    print(nnet)
    print("ConvTasNet #param: {:.2f}".format(param(nnet)))
...
if __name__ == "__main__":
    ...
    '''
    x = th.rand(16, 80000)
    cpt = th.load('/media/lab70809/Data02/MS_R1_SL1_saved_models/best.pt.tar')
    nnet=MB_ConvTasNet()
    nnet.load_state_dict(cpt["model_state_dict"])
    print("ConvTasNet #param: {:.2f}".format(param(nnet)))
    x = nnet(x)
