...
def foo_MB_ConvTasNet():
    ...
    x = nnet(x)
    s1 = x[0]
    summary(MB_ConvTasNet)
    print(s1.shape)
    trainable = sum([param.nelement() for param in nnet.parameters() if param.requires_grad])
    trainable1 = sum(p.numel() for p in nnet.parameters() if p.requires_grad)
    nontrainable = sum([param.nelement() for param in nnet.parameters()])
    print("trainable param",trainable)
    print("trainable param",trainable1)
    print("nontrainable param",nontrainable)

def foo_MS_SL2():
    x = th.rand(2, 50)
    nnet = MS_SL2_model(norm="cLN", causal=False)

    summary(nnet, input=x)
    print(nnet)
    print("ConvTasNet #param: {:.2f}".format(param(nnet)))
    x = nnet(x)
    s1 = x[0]
