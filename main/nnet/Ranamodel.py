...
class MS_SL2_split_model(nn.Module):
    def __init__(self,
        ...
        self.enocder_stft = ConvSTFT(win_len, win_inc, fft_len, win_type='hanning', feature_type='real')
        # keep T not change
        # T = int((xlen - L) / (L // 2)) + 1
    ...
    def forward(self, x):
        ...
        # n x 1 x S => n x N x T
        print(x.dim())
        en_1d = F.relu(self.encoder_1d(x))
        print("value of 1D encoder",en_1d)
        en_stft = F.relu(self.enocder_stft(x)[0])
