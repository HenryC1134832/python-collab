...
class BPFModule(nn.Module):
    ...
    def forward(self, time, tifr):
        time = time
        # time = th.as_tensor(time)
        print(time.shape)
        tifr = tifr
        # tifr = th.as_tensor(tifr)
        print(tifr.shape)
        # f_cube = self.CE.C2E(cube)
        f_cat = th.cat((time, tifr),1)
...
class EmbeddingLayer(nn.Module):
    ...
    def __init__(self, input_dim, embedding_dim):
        ...
        self.conv1 = nn.Conv1d(input_dim, 512, kernel_size=1)  # Conv1d適合序列資料
        self.conv2 = nn.Conv1d(512, embedding_dim, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
        self.linear = nn.Linear(embedding_dim, embedding_dim)  # 嵌入維度變換
        
    def forward(self, x):
        # 維度轉換適合 Conv1d: (batch_size, features, time_steps)
        x = self.relu(self.conv1(x))
        x = self.relu(self.conv2(x))
        
        # 線性層操作並還原維度
        batch_size, features, time_steps = x.size()
        x = x.permute(0, 2, 1)  # 變為 (batch_size, time_steps, features)
        x = self.linear(x)
        return x.permute(0, 2, 1)  # 回復為 (batch_size, features, time_steps)
...
class MS_SL2_split_model(nn.Module):
    def __init__(self,
        ...
        # 新增嵌入層
        self.embedding_layer = EmbeddingLayer(input_dim=512, embedding_dim=512)

        self.reduce_dim_conv = nn.Conv1d(1024, 512, kernel_size=1)
    ...
    def forward(self, x):
        ...
        w = self.encoder_1d(x)
        print('after encoder 1-D size', w.size())
        w = th.unsqueeze(w,2)
        w = th.squeeze(w,2)
        w = w[:, :256, :] 
        print('after stft_tensor N size', w.size())
        ...
        # 使用嵌入層生成新特徵
        embedding_features = self.embedding_layer(w)

        # 將嵌入特徵與原始特徵結合
        w = th.cat((w, embedding_features), dim=1)

        w = self.reduce_dim_conv(w)

        if __name__ == "__main__":
            print("output shape after embedding concatenation", w.shape)
