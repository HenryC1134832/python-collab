...
class MS_SL2_split_model(nn.Module):
    ...
    def _build_slices(self, num_slice, num_repeats, num_blocks, **block_kwargs):
        ...
        slices = [
         ...
         #Code for freezing first two slices and allow slice three to train 
        #for param in slices[0].parameters():
         #   param.requires_grad = False
        #for param in slices[1].parameters():
            #param.requires_grad = False

        #Allow the third slice for training
        #trainable_slices = [slices[2]]
        #end of transfer learing code
...
class MS_SL2_attention_model(nn.Module):
    def __init__(self,
                 L=16, #length of filters(in sample) 
                 N=512, #Number of filters in Autoencoder
                 X=8, #Number of convolution block in each repeat
                 ...
                 non_linear="sigmoid",
                 causal=False):
        super(MS_SL2_attention_model, self).__init__()
        supported_nonlinear = {
            "relu": F.relu,
            "sigmoid": th.sigmoid,
            "softmax": F.softmax
...
if __name__ == "__main__":
    ...
    x = th.rand(4, 1000)
    nnet = MS_SL2_attention_model(norm="cLN", causal=False)
    print(nnet)
    print("ConvTasNet #param: {:.2f}".format(param(nnet)))
    x = nnet(x)
    s1 = x[0]
