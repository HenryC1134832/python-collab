...
class MS_SL2_split_model(nn.Module):
    def __init__(self,
        ...
        #weight for each branch
        self.wList = nn.Parameter(th.tensor([0.5+0.001, 0.4-0.001]),requires_grad=True)
        # self.wList = nn.Parameter(th.tensor([0.5+0.001, 0.4-0.001,0.5-0.001,0.4+0.001]),requires_grad=True)
    ...
    def forward(self, x):
        ...
        for Slice in range(self.slice):
            ...
            for i in range (len(self.wList)): 
                ...
                total_connection=0
            # print("weight size", self.wList.size())   
...
#     cpt = th.load('/media/speechsever/Toshibha-3.0TB/MS_R1_SL2_saved_models/15.pt.tar')
