...
class MB_ConvTasNet(nn.Module):
    def __init__(self,
        ...
        self.wList = self._build_weight(R)
    ...
    def _build_weight(self, R):
        wList = []
        for i in range(R):
            wList.append(nn.Parameter(th.randn(1,requires_grad=True)))
    ...
    def _build_branch(self, num_repeats, num_blocks, **block_kwargs):
        """
        Build Conv1D block repeats
        """
        repeats = [
    ...
    def forward(self, x):
        ...
        for i in range(self.R):
            for j in range(self.X):
                skip, y = self.branch[i][j](y)
                skip_connection = skip_connection + skip
            
            total_connection = skip_connection + self.wList[i] * total_connection
            print(self.wList[i])
            print(total_connection.size())
            skip_connection = 0
