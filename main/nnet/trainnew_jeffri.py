...
import torch as th
import torchvision
from libs.trainer_embedding import SiSnrTrainer
...
from model_SC_CHM_Fusion_embedding import MS_SL2_split_model
...
def run(args):
    ...
    # 如果提供了resume參數，則從檢查點恢復模型，進行微調
    trainer = SiSnrTrainer(nnet,
                           gpuid=gpuids,
                           checkpoint=args.checkpoint,
                           resume=args.resume,  # 使用傳入的resume參數
                           **trainer_conf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume",
                        type=str,
                        default="",
                        help="Exist model to resume training from")  # 新增此行來接受resume參數
