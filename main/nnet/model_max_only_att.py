...
class AttentionDn(nn.Module):
    def __init__(self, dnc_dim=128):
        super(AttentionDn, self).__init__()
        self.conv = nn.Conv1d(dnc_dim, dnc_dim, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn = ChannelWiseLayerNorm(dnc_dim)
        self.drop = nn.Dropout(0.2)
        self.softmax = nn.Softmax(dim=-1)
...
class MS_SL1_only_attention_model(nn.Module):
    def __init__(self,
        ...
        self.attentionEn = AttentionEn(N)
        # keep T not change
        # T = int((xlen - L) / (L // 2)) + 1
    ...
    def forward(self, x):
        ...
        #   LayerNorm & 1X1 Conv
        # n x B x T
        w = self.attentionEn(w)
        if __name__ == "__main__":
            print('after attention size', w.size())
            print('attention sum:',sum(m.numel() for m in self.attentionEn.parameters()))
