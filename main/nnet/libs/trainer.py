    ...
    def __init__(self,
        ...
        if resume:
            if not os.path.exists(resume):
                raise FileNotFoundError(f"Could not find resume checkpoint: {resume}")
            cpt = th.load(resume, map_location="cpu")
            self.cur_epoch = 0
            self.logger.info(f"Resume from checkpoint {resume}: epoch {self.cur_epoch}")
        ...
        # 解凍 Fusion-Core 的參數
        self.nnet.unfreeze_fusion_core()

        print('patience: ', patience)
        self.scheduler = ReduceLROnPlateau(self.optimizer, mode="min", factor=factor, patience=patience, min_lr=min_lr, verbose=True)
        ...
        if clip_norm:
            self.logger.info(f"Gradient clipping by {clip_norm}, default L2")
